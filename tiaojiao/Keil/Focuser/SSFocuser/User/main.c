...
bool  bIsHome         = 0; 
bool  bIsSleeping    = true;      //Drv8825是否处于睡眠模式
...
u16   uTimerCount=0;
u16   uSecondCount=0;
u16   uDelayCycle=180;//不操作进入睡眠模式时间
...
//设置细分
void SetDivision2(u8 uDrvNum,u8 sMode)
{
	if(uDrvNum==0)
	{
		switch(sMode)
...
void SlewOutSteps(u32 Steps)//
{
	iStepStop=(iStepCount/uSubdivision+Steps)*uSubdivision;
	if(uIsMoving == 1)
		;
	else
	{
...
void SlewInSteps(u32 Steps)//
{
	iStepStop=(iStepCount/uSubdivision-Steps)*uSubdivision;//多设置500步保证500mS内不会停掉
	if(uIsMoving == 2)
		;
	else
	{
...
bool CmdProcess(u8 MyComPort,unsigned char *RxBuffer,unsigned char *Ptr)
{
	...
	if(CmdBuff[0]=='F')//调焦器命令以F开头
	{
		switch (CmdBuff[1])
		{
			...
			case 'L': //设置功耗模式，自动识别可以不用，只有DRV8825使用低功耗模式		
				{
					sprintf(ReplyBuff, ":L#\r\n");
					SendTo(MyComPort,ReplyBuff);
					/*
...
{
	u8 Value=0;
	if(GPIO_ReadInputDataBit(HU_GPIO_PORT,HU_GPIO_PIN)==1) 
		Value+=1;
	if(GPIO_ReadInputDataBit(HD_GPIO_PORT,HD_GPIO_PIN)==1) 
		Value+=2;
	return Value;
}
//Key事件函数
void KeyEvent(void)
{
	...
	if(Key==KeyPrv)//
	{
		if(Key==0)
		{
			if(KeySpeed)
			{
				if(wait_ms(1000,uHanderCountPrv))
				{
					if(uSpeed<uSpeedMax)
						uSpeed+=1;
					else
	...
	{
		KeyTmp=Key;
		delay_ms(100);//100mS消除抖动
		Key=KeyValue();
		if(Key==KeyTmp)
		{
			KeyPrv=Key;
			uHanderCountPrv=uTimerCount;
			if(Key==0)	
			{
				Halt();
				KeySpeed=true;
...
{	
	...
	{	
	  ...
	  { 
		...
		if((bIsLowPower)&&(!bIsSleeping)&&(uIsMoving==0))
		{
