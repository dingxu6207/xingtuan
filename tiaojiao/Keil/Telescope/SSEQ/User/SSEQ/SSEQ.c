...
#define LOW 0
int WORM = 144;          //蜗杆减速比
float REDUCTOR = 4;      //齿轮减速比
int DRIVE_STP = 200;   	 //步进电机分辨率，每圈多少步
int MICROSteps = 16;     //步进电机细分数

// below variables are used to calculate the paramters where the drive works
int ARCSEC_F_ROTAT = 1296000;      //每圈角秒数360*3600
float SIDEREAL_DAY = 86164.0905;   //每个恒星日秒数23*3600+56*60+4.09
float ArcSECstep;
int MicroSteps_360;      //360度对应的步数，含细分
int RA_90;               //90度对应的步数
int DEC_90;   					 //90度对应的步数
float HA_H_CONST;        //每度对应的步数
int HA_M_CONST;          
float DEC_D_CONST;       //每度对应的步数
int DEC_M_CONST;
...
//
//
const char * FirmwareDate = "03 08 17";
const char * FirmwareNumber = "v2.3.1 Boiana EQ";
const char * FirmwareName = "rDUINOScope";
const char * FirmwareTime = "12:00:00";
//
int sd_cs = 42;               // TFTShield sd card chip select pin is Arduino digital pin 42
char * Messier_Array[120];
char * Treasure_Array[130];
char * ObservedObjects[50];
char * Iter_Stars[50];
...
char * Stars[] = {            // Definition of the ALIGNMENT Stars. Add as many stars as you like in here
...
bool IS_TFT_ON = true;
bool IS_STEPPERS_ON = true;
bool IS_OBJ_VISIBLE = false;
bool IS_IN_OPERATION = false;  // This variable becomes True when Main screen appears
bool IS_TRACKING = false;
bool IS_NIGHTMODE;
bool IS_OBJ_FOUND = true;
bool IS_OBJECT_RA_FOUND = true;
bool IS_OBJECT_DEC_FOUND = true;
bool IS_MERIDIAN_PASSED = false;
bool IS_POSIBLE_MERIDIAN_FLIP = true;
bool IS_MERIDIAN_FLIP_AUTOMATIC = true;
bool IS_BT_MODE_ON = false;
bool IS_MANUAL_MOVE = false;
bool IS_DEV1_ON = false;
bool IS_DEV2_ON = false;
bool IS_CUSTOM_MAP_SELECTED = false;
...
//  HA = LST - RA 
    char * Date_q = String(rtc.getDateStr());
    char * Time_q = String(rtc.getTimeStr());
...
void selectOBJECT_M(int index_, int objects) {
   ...
   if (objects == 0){                                           // I've selected a Messier Object
      ...
      char * OBJ_RA = Messier_Array[index_].substring(i1,i2);
      ...
      char * OBJ_DEC = Messier_Array[index_].substring(i2,i3);
      char * sign = OBJ_DEC.substring(1,2);
      ...
      char * OBJ_RA = Treasure_Array[index_].substring(i1,i2);
      OBJECT_RA_H = OBJ_RA.substring(1,OBJ_RA.indexOf('h')).toFloat();
      OBJECT_RA_M = OBJ_RA.substring(OBJ_RA.indexOf('h')+1,OBJ_RA.length()-1).toFloat();
      char * OBJ_DEC = Treasure_Array[index_].substring(i2,i3);
      char * sign = OBJ_DEC.substring(1,2);
      OBJECT_DEC_D = OBJ_DEC.substring(2,OBJ_DEC.indexOf('°')).toFloat();
      OBJECT_DEC_M = OBJ_DEC.substring(OBJ_DEC.indexOf('°')+1,OBJ_DEC.length()-1).toFloat();
      if (sign == "-"){ 
      ...
      char * OBJ_RA = Stars[index_].substring(i2+1,i3);
      OBJECT_RA_H = OBJ_RA.substring(0,OBJ_RA.indexOf('h')).toFloat();
      OBJECT_RA_M = OBJ_RA.substring(OBJ_RA.indexOf('h')+1,OBJ_RA.length()-1).toFloat();
      char * OBJ_DEC = Stars[index_].substring(i3,Stars[index_].length());
      char * sign = OBJ_DEC.substring(0,1);
      ...
      char * OBJ_RA = Iter_Stars[index_].substring(i2+1,i3);
      OBJECT_RA_H = OBJ_RA.substring(0,OBJ_RA.indexOf('h')).toFloat();
      OBJECT_RA_M = OBJ_RA.substring(OBJ_RA.indexOf('h')+1,OBJ_RA.length()-1).toFloat();
      char * OBJ_DEC = Iter_Stars[index_].substring(i3,Iter_Stars[index_].length());
      char * sign = OBJ_DEC.substring(0, 1);
      OBJECT_DEC_D = OBJ_DEC.substring(1,OBJ_DEC.indexOf('°')).toFloat();
      if (sign == "-"){ 
          OBJECT_DEC_D *= (-1);
...
//微小延迟 Keep the GPS sensor "fed" until we find the data.
